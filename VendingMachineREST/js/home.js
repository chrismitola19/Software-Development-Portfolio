...
function loadItems() {
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i += COLUMNS) {
                var rowString = '';
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.quarters > 0) {
                changeString = changeString.concat('Quarter(s): ' + response.quarters + ' ');
            }
            if (response.dimes > 0) {
...
$(document).ready(function () {
    ...
    buttonReturnChange.click(function () {
        ...
        var changeString = '';
        if (quarters > 0) {
            changeString = changeString.concat('Quarter(s): ' + quarters + ' ');
        }
        if (dimes > 0) {
